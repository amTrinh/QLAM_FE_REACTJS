...
function Artist() {
  ...
  //
  const [SongartistData, setSongs] = useState([]);
  useEffect(() => {
    artistApi.getArtistById(artist.id).then((res) => {
      setSongs(res.data);
    });
  }, []);
  // console.log(SongartistData);
  const artistSongs = SongartistData.songs;
  //
  ...
  //   });
  // });
  const artistAlbums = SongartistData.albums;
  ...
  //   });
  // });
  if (artistSongs != null && artistAlbums != null){
return (
  <div>
    {artist && (
